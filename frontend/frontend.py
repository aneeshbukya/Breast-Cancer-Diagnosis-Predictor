...
def add_predictions(input_data: Dict[str, float]) -> None:
    ...
    if input_array.shape[1] != scaler.n_features_in_:
        st.error(f"Error: Input has {input_array.shape[1]} features, but scaler expects {scaler.n_features_in_} features.")
        return
